...
const listMaker= ()=>{
    ...
    return{discount, numberOfVisit: countNumber};
}

const calculateDiscount=(discount)=>{
    var cleanNumberPrice = parseFloat(originalPrice.replace(",",".").replace("TL","").trim());
    var calculatedPrice = (cleanNumberPrice - (cleanNumberPrice/100*discount)).toFixed(2);
...
const deleteHandler= (popUp)=>{
    body.removeChild(popUp);
}

const giveDeleteHandlers= (popUp)=>{
    popUp.querySelector(".closeButton").addEventListener("click",()=> deleteHandler(popUp));
    popUp.addEventListener("click",()=> deleteHandler(popUp));
...
const initialize = ()=>{
    clearCodes();
    addStyleAndCdn();
    const {discount, numberOfVisit} = listMaker();
    const calculatedPrice = calculateDiscount(discount);
    if(numberOfVisit>1){
        changePrice(calculatedPrice);
        const popUp = showPopUp(numberOfVisit,discount,originalPrice);
        giveDeleteHandlers(popUp);
    }
}

if(productCode)initialize();